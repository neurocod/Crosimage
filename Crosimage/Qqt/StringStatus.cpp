...
QString StringStatus::toString()const {
	if(!_error)
		return QString("Status: OK");
	return QString("Status: error %1").arg(*_error);
}
...
void StringStatus::msgBox(const QString & additionalInfo)const {
	...
	if(_error) {
		...
		QMessageBox::critical(0, title, *_error);
		return;
	}
	QMessageBox::information(0, qApp->applicationName(), additionalInfo);
#else
	if(_error) {
		QString s = *_error;
		if(!additionalInfo.isEmpty())
